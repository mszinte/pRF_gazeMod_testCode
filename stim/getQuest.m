function [expDes,stim] = getQuest(const,expDes,stim,current_time,t)
% ----------------------------------------------------------------------
% [expDes,stim] = getQuest(const,expDes,stim,current_time,t)
% ----------------------------------------------------------------------
% Goal of the function :
% Get the quest values
% ----------------------------------------------------------------------
% Input(s) :
...
if const.typeTask == 2
    ...
    for tBin = 1:const.nr_staircases_ecc
        if tBin < const.nr_staircases_ecc
            if meanDist >= const.valBin(tBin) && meanDist < const.valBin(tBin+1)
    ...
    if stim.pulse
        % color task
        expDes.quest_sample_color       = QuestQuantile(expDes.quest_qColor{stim.eccentricity_bin+1});
        if expDes.quest_sample_color < const.quest_grain;expDes.quest_sample_color = const.quest_grain;end
        expDes.color_1_ratio            = 1 - (1/(exp(1)^expDes.quest_sample_color+1));
        ...
        log_txt = sprintf('signal in feature: Color ecc bin: %i phase: %1.3f value: %f at %f',stim.eccentricity_bin,stim.current_phase,expDes.quest_sample_color,current_time);
        fprintf(const.log_text_fid,'%s\n',log_txt);                                                                         % write in log
        if const.tracker;Eyelink('message','%s',log_txt);end                                                                % write in edf
        
        % save data
        expDes.resColor = [expDes.resColor;expDes.expMat(t,:),stim.eccentricity_bin,expDes.quest_sample_color,current_time,NaN,NaN];
...
elseif const.typeTask == 3
    ...
    if stim.pulse
        % color task
        expDes.quest_sample_fix       = QuestQuantile(expDes.quest_qFix);
        if expDes.quest_sample_fix < const.quest_grain;expDes.quest_sample_fix = const.quest_grain;end
        expDes.fix_value                = ((1 - (1/(exp(1)^expDes.quest_sample_fix+1)))- 0.5) * 2.0;
        ...
        log_txt = sprintf('signal in feature: Fix value: %f at %f',expDes.quest_sample_fix,current_time);
        fprintf(const.log_text_fid,'%s\n',log_txt);                                                                         % write in log
        if const.tracker;Eyelink('message','%s',log_txt);end                                                                % write in edf
        
        % save data
        expDes.resFix = [expDes.resFix;expDes.expMat(t,:),expDes.quest_sample_fix,current_time,NaN,NaN];
        % rows = pulse occurences
        % col1 = block number
        % col2 = trial number
        % col3 = type of task (0 = fixation; 1 = fixation no stim)
        % col4 = pulse presence (0 = present; 1 = absent)
        % col5 = staircase color quest value
        % col6 = signal time
